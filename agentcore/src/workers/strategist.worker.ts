...
export function createStrategistWorker(tenantId: string): Worker {
  return new Worker(
    getQueueName(tenantId, 'strategist'),
    async (job: Job) => {
      let task: { id: string; tenantId: string } | undefined;
      try {
        task = await createTaskRecord(job, 'strategist');
      } catch (taskErr) {
        logger.warn({ err: taskErr, tenantId, jobId: job.id }, 'Failed to create task record for strategist -- continuing without it');
      }
      ...
      try {
        ...
        logger.error({ err, tenantId, jobId: job.id }, 'Strategist worker failed');
        throw err;
      } finally {
        await agent.close();
      }
    },
    {
      ...
      concurrency: QUEUE_CONFIGS['strategist'].concurrency,
    },
  );
}
